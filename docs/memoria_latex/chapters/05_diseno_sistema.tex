...
El código generado no debe redactar la respuesta final. Su responsabilidad es calcular métricas y devolver una salida estructurada. El contrato mínimo exige una función \texttt{main()}, lectura del payload recibido por \texttt{argv[1]}, escritura de un único JSON por \texttt{stdout} y uso exclusivo de imports permitidos. Estas reglas permiten que el sistema trate todos los scripts generados de la misma forma: primero los valida, después los ejecuta en un proceso separado y finalmente interpreta su salida JSON.
